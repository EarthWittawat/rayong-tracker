...
// Invoked by the client after inserting a comment.
// Looks up @mentioned users + subscribers (excluding the commenter),
// writes per-recipient rows into `public.notifications`, then sends
// real-time emails via Resend for users whose `notify_mentions` /
// `notify_replies` preferences are true.
//
// Deploy:
...
//
// The function is invoked from the client via supabase.functions.invoke('send-mail', {body: {...}})
// so JWT verification is left on (default). The function uses the SUPABASE_SERVICE_ROLE_KEY
// (auto-injected) to bypass RLS when fanning out notification rows.
...
Deno.serve(async (req: Request) => {
  ...
  if (!payload?.comment_id) return new Response("missing comment_id", { status: 400 });
  ...
  // Fan-out recipient set: mentions ∪ subscribers, minus author
  ...
  // Profiles for recipients + pref filtering
  ...
  for (const p of profiles ?? []) {
    ...
    // Always log the notification (drives the digest + in-app indicator later).
  ...
  return new Response(JSON.stringify({ ok: true, queued, sent }), {
    headers: { "Content-Type": "application/json" },
  });
});
